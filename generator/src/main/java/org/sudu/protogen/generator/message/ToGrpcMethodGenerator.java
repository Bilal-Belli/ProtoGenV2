...
public class ToGrpcMethodGenerator {
    ...
    public CodeBlock builder(String builderName) {
        ...
        List<CodeBlock> fieldTransformers = processedFields.stream()
                .map(p -> {
                    if (p.original().getNeedGetterInOriginalClass()) {
                        return new FieldTransformerGenerator(p.type(), p.original().getName(), p.isNullable())
                                .toGrpc(builderName, p.original().getGetterMethodName()+"()");
                    } else {
                        return new FieldTransformerGenerator(p.type(), p.original().getName(), p.isNullable())
                                .toGrpc(builderName, p.field().name);
                    }
                })
                .toList();
